...
/*      added by T. DECJAIZE : because many warnings appears without :
...
GLenum tkInitWindowAW(char *title, BOOL bUnicode)
{
    WNDCLASS wndclass;
    RECT     WinRect;
/*      Change by Thierry DECHAIZE :   HANDLE   hInstance;    by   HINSTANCE Instance; */
    HINSTANCE hInstance;
    ATOM     aRegister;
...
/*      Change by Thierry DECHAIZE :   add cast (HPALETTE)  before call of AGetStockObject  */
...
// Cleanup the palette.
    ...
    // If static system color usage is set, restore the system colors.
        ...
        if ( tkUseStaticColors )
        {
/*      Change by Thierry DECHAIZE :   add cast (HPALETTE)  before call of AllocateMemory   */
