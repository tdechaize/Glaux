...
/*   Change by Thierry DECHAIZE :  about syntax "new[0] = old[0], new[1] = old[1], new[2] = old[2];"
     WARNING :  With GCC, "new" is recognize as key word of language C/C++ => error compiler.
                But, with option "-x c" during GCC compilation, you can force generation
                with use of strict C language, and you can force also alignment with
                standard norm of C with use of "-std=c99" option by example => success build.
*/
