...
static void APIENTRY RawImageClose(rawImageRec *raw);

static rawImageRec APIENTRY *RawImageOpenAW(char *fileName, BOOL bUnicode)
{
    rawImageRec *raw;
...
/*              Change by Thierry DECHAIZE : raw->rowSize = (long *)malloc(x); -> raw->rowSize = (unsigned long *)malloc(x);  */
...
static void APIENTRY RawImageGetRow(rawImageRec *raw, unsigned char *buf, int y, int z)
{
...
static void APIENTRY RawImageGetData(rawImageRec *raw, TK_RGBImageRec *final)
{
    unsigned char *ptr;
    int i, j;
...
TK_RGBImageRec APIENTRY *tkRGBImageLoad(char *fileName)
{
    return tkRGBImageLoadAW(fileName, FALSE);
}

TK_RGBImageRec APIENTRY *tkRGBImageLoadAW(char *fileName, BOOL bUnicode)
{
    rawImageRec *raw;
