...
void APIENTRY tkErrorPopups(GLboolean bEnable)
{
    tkPopupEnable = bEnable;
}

void APIENTRY tkCloseWindow(void)
{
    DestroyThisWindow(tkhwnd);
}


void APIENTRY tkExec(void)
{
    MSG Message;
...
void APIENTRY tkExposeFunc(void (*Func)(int, int))
{
    ExposeFunc = Func;
}

void APIENTRY tkReshapeFunc(void (*Func)(GLsizei, GLsizei))
{
    ReshapeFunc = Func;
}

void APIENTRY tkDisplayFunc(void (*Func)(void))
{
    DisplayFunc = Func;
}

void APIENTRY tkKeyDownFunc(GLenum (*Func)(int, GLenum))
{
    KeyDownFunc = Func;
}

void APIENTRY tkMouseDownFunc(GLenum (*Func)(int, int, GLenum))
{
    MouseDownFunc = Func;
}

void APIENTRY tkMouseUpFunc(GLenum (*Func)(int, int, GLenum))
{
    MouseUpFunc = Func;
}

void APIENTRY tkMouseMoveFunc(GLenum (*Func)(int, int, GLenum))
{
    MouseMoveFunc = Func;
}

void APIENTRY tkIdleFunc(void (*Func)(void))
{
    IdleFunc = Func;
}

void APIENTRY tkInitPosition(int x, int y, int width, int height)
{
    if (x == CW_USEDEFAULT)
    {
        x = 0;
...
void APIENTRY tkInitDisplayMode(GLenum type)
{
    windInfo.type = type;
}

void APIENTRY tkInitDisplayModePolicy(GLenum type)
{
    windInfo.dmPolicy = type;
}

GLenum APIENTRY tkInitDisplayModeID(GLint ipfd)
{
    windInfo.ipfd = ipfd;
    return GL_TRUE;
}
...
GLenum APIENTRY tkInitWindow(char *title)
{
...
WARNING : GetVersionEx (and derived functions GetVersionExA or GetVersionExW) is deprecated since Windows 10

Replace it by VerifyVersionInfoA and use of VER_SET_CONDITION() macro :
...
Or better use d’assistance's Functions of version :
...
Pour Windows 8,
IsWindows8Point1OrGreater retourne false, sauf si l’application contient un manifeste qui inclut une section de compatibilité qui contient les GUID qui désignent Windows 8.1 et/ou Windows 10.
...
Pour Windows 10,
IsWindows10OrGreater retourne false, sauf si l’application contient un manifeste qui inclut une section de compatibilité qui contient le GUID qui désigne Windows 10.
...
IsWindowsVersionOrGreater 	Vous ne devez utiliser cette fonction que si les autres fonctions d’assistance de version fournies ne correspondent pas à votre scénario.
                            Indique si la version actuelle du système d’exploitation correspond ou est supérieure aux informations de version fournies.
                            Cette fonction est utile pour confirmer une version de Windows Server qui ne partage pas de numéro de version avec une version cliente.
...
GLenum APIENTRY tkInitWindowAW(char *title, BOOL bUnicode)
{
    WNDCLASS wndclass;
    RECT     WinRect;
/*      Change by Thierry DECHAIZE :   HANDLE   hInstance;    ->   HINSTANCE   Instance; */
    HINSTANCE hInstance;
    ATOM     aRegister;
...
/*      Change by Thierry DECHAIZE :   add cast (HBRUSH)  before call of GetStockObject    */
...
void APIENTRY tkSetOneColor(int index, float r, float g, float b)
{
...
void APIENTRY tkSetFogRamp(int density, int startIndex)
{
    HPALETTE CurrentPal;
...
/*      Change by Thierry DECHAIZE :   add cast (PALETTEENTRY *)  before call of AllocateMemory   */
        pPalEntry = (PALETTEENTRY *)AllocateMemory( n * sizeof(PALETTEENTRY) );
...
void APIENTRY tkSetGreyRamp(void)
{
    HPALETTE CurrentPal;
...
/*      Change by Thierry DECHAIZE :   add cast (PALETTEENTRY *)  before call of AllocateMemory   */
        Entries = (PALETTEENTRY *)AllocateMemory( Count * sizeof(PALETTEENTRY) );
...
void APIENTRY tkSetRGBMap( int Size, float *Values )
{
    HPALETTE CurrentPal;
...
void APIENTRY tkSwapBuffers(void)
{
    SwapBuffers(tkhdc);
}
...
GLint APIENTRY tkGetColorMapSize(void)
{
...
void APIENTRY tkGetMouseLoc(int *x, int *y)
{
    POINT Point;
...
HWND APIENTRY tkGetHWND(void)
{
    return tkhwnd;
}

HDC APIENTRY tkGetHDC(void)
{
    return tkhdc;
}

HGLRC APIENTRY tkGetHRC(void)
{
    return tkhrc;
}

GLenum APIENTRY tkGetDisplayModePolicy(void)
{
    return windInfo.dmPolicy;
}

GLint APIENTRY tkGetDisplayModeID(void)
{
    return windInfo.ipfd;
}

GLenum APIENTRY tkGetDisplayMode(void)
{
...
/*      Change by Thierry DECHAIZE :   add cast (HPALETTE)  before call of GetStockObject   */
    ...
    if ( NULL == tkhpalette )
    {
        ...
        if ( PixelFormatDescriptorFromDc( Dc, pPfd ) )
        {
                 ...
                 (pPfd->dwFlags & PFD_NEED_PALETTE) )
            {
                ...
                Count       = 1 << pPfd->cColorBits;
/*      Change by Thierry DECHAIZE :   add cast (LOGPALETTE *)  before call of AllocateMemory   */
                LogPalette  = (LOGPALETTE *)AllocateMemory( sizeof(LOGPALETTE) +
...
/*      Change by Thierry DECHAIZE :   add cast (LOGPALETTE *)  before call of AllocateMemory   */
                LogicalPalette = (LOGPALETTE *)AllocateZeroedMemory( sizeof(LOGPALETTE) +
                ...
                if ( NULL != LogicalPalette )
                {
                    ...
                    StockPalette     = (HPALETTE)GetStockObject(DEFAULT_PALETTE);
...
// Cleanup the palette.
    ...
    // If static system color usage is set, restore the system colors.
        ...
        if ( tkUseStaticColors )
        {
            RealizePaletteNow( tkhdc, (HPALETTE)GetStockObject(DEFAULT_PALETTE), TRUE );
        }
        else
        {
